...
fn main() {
    ...
    let mut publ = if args.exclusive {
        ...
        Pub::Exclusive(client.add_exclusive_publication(PING_CHANNEL, PONG_STREAM_ID).unwrap())
    } else {
        Pub::Regular(client.add_publication(PING_CHANNEL, PONG_STREAM_ID).unwrap())
    };
