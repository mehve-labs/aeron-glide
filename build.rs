...
fn main() {
    ...
    if !aeron_dir.exists() {
        let url = format!("https://github.com/real-logic/aeron/archive/refs/tags/{}.tar.gz", aeron_version);
        download_and_extract(&url, &out_dir);
    }

    // Build Aeron C++ using CMake
    let mut config = Config::new(&aeron_dir);
    config.define("BUILD_AERON_DRIVER", "ON")
...
fn download_and_extract(url: &str, dest_dir: &PathBuf) {
    ...
    archive.unpack(dest_dir).expect("Failed to unpack Aeron archive");
}
